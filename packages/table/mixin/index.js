...
export default {
  props: {
    ...
    recordTableSelect: {
      default: false,
      type: Boolean
    }, // 是否记录表格的选项id(必须保证行ID存在，且唯一)
    headerDragStyle: {
      default: false,
      type: Boolean
    }, // 是否修改表格的头部拖动样式  | Boolean | false
    useVirtual: {
      default: false,
      type: Boolean
    }, // 是否开启虚拟滚动 | Boolean | false
    rowHeight: {
      default: 60,
      type: Number
    }, // 行高(必须要设置行高，否则会导致表格计算不正确)| Number | 60
    excessRows: {
      default: 3,
      type: Number
    }, //	可视区域上方和下方额外渲染的行数，行数越多表格接替渲染效果越好，但越耗性能	Number	3
    ...
    tooltipPlacement: {
      default: 'top',
      type: String
    }, // Tooltip 的出现位置	String	top
    summaryMethod: Function, // 自定义的合计计算方法
    bigDataCheckbox: {
      default: false,
      type: Boolean
    }, // 大量数据时候（至少数据量大于3000条每页），表格存在selection选择列表时，是否开启解决全选卡顿（缓慢）问题，开启此项可能影响table-column的selectable属性功能，最好不要使用selectable
  ...
  methods: {
    ...
    toggleRowExpansion(row, expanded) {
      this.$refs.singlePlTable.toggleRowExpansion(row, expanded);
    },
    // 合并行或列的计算方法
    arraySpanMethod({
      row,
    ...
    toggleRowSelection(rows) {
      var arrRows = [];
      if (rows) {
        if (this.isArrayFn(rows)) {
          arrRows = rows.map((item) => {
            return {row: item};
          });
        } else {
          arrRows = [{row: rows}];
      ...
      this.$refs.singlePlTable.toggleRowSelection(arrRows);
    },
    // 用于多选表格，切换所有行的选中状态
    toggleAllSelection() {
      if (this.$refs.singlePlTable) {
        this.$refs.singlePlTable.toggleAllSelection();
      } else {
    ...
    clearSelection() {
      if (this.$refs.singlePlTable) {
        this.$refs.singlePlTable.clearSelection();
      } else {
    ...
    pagingScrollTopLeft(top = 0, left = 0) {
      this.$refs.singlePlTable.pagingScrollTop(top, left);
    },
    // 滚动表格到底部
    scrollBottom() {
      this.$refs.singlePlTable.scrollBottom();
    },
    // 合计的方法
    getSummaries(param) {
    ...
    rowClick(row, column, event) {
      if (this.highlightCurrentRow) {
        this.$refs.singlePlTable.setCurrentRow(row);
      }
        ...
        event.path.forEach(item => {
          if (item && item.classList) {
            item.classList.forEach(its => {
              if (its === 'current-row') {
                this.$refs.singlePlTable.setCurrentRow();
              }
            });
          }
        });
    ...
    doLayout() {
      this.$refs.singlePlTable.doLayout();
    },
    // 删除节点
    clearNode() {
      // 删除节点
    ...
    clearSort() {
      this.$refs.singlePlTable.clearSort();
    },
    // 不传入参数时用于清空所有过滤条件，数据会恢复成未过滤的状态
    clearFilter(columnKey) {
      this.$refs.singlePlTable.clearFilter(columnKey);
    },
    // 手动对 Table 进行排序
    sort(prop, order) {
      this.$refs.singlePlTable.sort(prop, order);
    },
    ...
    isTreeExpandByRow(row) {
      this.$refs.singlePlTable.isTreeExpandByRow(row);
    },
    // 展开全部pl-table的树
    setAllTreeExpansion() {
      this.$refs.singlePlTable.setAllTreeExpansion();
    },
    // 关闭所有
    clearTreeExpand() {
      this.$refs.singlePlTable.clearTreeExpand();
    },
    // 获取已展开的节点
    getTreeExpandRecords() {
      return this.$refs.singlePlTable.getTreeExpandRecords();
    },
    // 设置展开树形节点
    setTreeExpansion(rows, expanded) {
      this.$refs.singlePlTable.setPlTreeExpansion(rows, expanded);
    },
    // 切换展开树形节点的状态
    toggleTreeExpansion(row) {
      this.$refs.singlePlTable.toggleTreeExpansion(row);
    },
    setHeight() {
      this.$refs.singlePlTable.setHeight();
    },
    reloadData(data) {
      this.$refs.singlePlTable.reloadData(data);
    }
  },
  // 生命周期结束
  beforeDestroy() {},
